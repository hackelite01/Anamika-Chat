...
mongoose.connect(`mongodb+srv://hackelite01:${process.env.DB_KEY}@anamika.ndkijnn.mongodb.net`);
...
app.get("/logout", (req, res) => {
    req.logout((err) => {
        if (err) {
            console.log(err);
            res.send("Something went wrong! Kindy retry or conatct Mayank Rajput.")
        } else {
            res.redirect("/");
